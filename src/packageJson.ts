...
function foundPackageJson(baseDir: string) {
    try {
        ...
        consoleCheckout([{ env: `PACKAGE.JSON NOT FOUND`, status: "info" }], baseDir);
    }
}
...
export function includetscInScriptStart(baseDir: string) {
    ...
    if (!currentPackageJson.scripts) {
        consoleCheckout([{ env: "SCRIPT START COMAND NOT FOUNT IN PACKAGE ❌", status: "info" }], baseDir)
        return
    }
    const startScript = currentPackageJson.scripts?.start
    if (!startScript) {
        consoleCheckout([{ env: "START COMAND NOT FOUNT ❌", status: "info" }], baseDir)
        return
    }

    if (startScript.includes("elc")) {
        consoleCheckout([{ env: "COMAND ALREDY INCLUDE ❌", status: "info" }], baseDir)
        return
    }
...
export function getLibsInDependencie(baseDir: string) {
    const currentPackageJson = foundPackageJson(baseDir)
    if (!currentPackageJson.scripts) {
        consoleCheckout([{ env: "SCRIPT START COMAND NOT FOUNT IN PACKAGE ❌", status: "info" }], baseDir)
        return
    ...
    if (!dependencies) {
        consoleCheckout([{ env: "DEPENDENCIES NOT FOUND IN PACKAGE.JSON ❌", status: "info" }], baseDir)
        return
    }
